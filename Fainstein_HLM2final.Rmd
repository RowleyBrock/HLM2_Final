...
## Procedure
  ODR data from the SWIS database were extracted following a formal, written data request submitted by this author. This dataset was obtained as a referral-level dataset and required conversion to a student-level dataset by running cases to variables command in SPSS. For all analyses of administrative actions taken in response to referrals, only a student’s first referral for substance possession was used, meaning any administrative actions in subsequent referrals were not included. Analysis was done using only the first referrals for substance possession for each student in the dataset to eliminate concerns about possible confounding of analyses related to decision-making with subsequent referrals. 

## Missing Data
Missing data was handled by listwise deletion for instances where an individual student's race or ethnicity was not available (i.e., listed as Not Applicable, NA). The race or ethnicity identifier was drawn from the National Center on Educational Statistics approach for collecting and disseminating extant data. In this case, 7.7% (2360 of 30773 observations) of the data was removed from analyses because there was insufficient demographic information.
...
## Priors
*Note that we used flat, preset priors from the brms package (cite)
...
More notes:
Use insight() to grab posterior distributions. 
...
# interaction model with specifications to get rhat down to 1.0 across the board
subxrace_brm <- brm(OSS_1stRef ~  1 + Drugs.1 + Tobacco.1 +
                    RaceEthnicity +
             ...
             iter = 3000,
             warmup = 1000,
             # 1000 warmup + 2000 sampling
             backend = "cmdstan")

subxrace_brm_3 <- brm(OSS_1stRef ~  1 + Alcohol.1 + Drugs.1 + Tobacco.1 +
                    RaceEthnicity +
                    RaceEthnicity:Drugs.1 + RaceEthnicity:Tobacco.1 + 
                      RaceEthnicity:Alcohol.1 +
                    (1|School),
             data= year7,
             family = bernoulli(link = "logit"),
             cores = 4,
...
summary(subxrace_brm)

#looking at probability per substance, race, and interaction
probabilities <- as.data.frame(brms::inv_logit_scaled(fixef(subxrace_brm)))

interaction_poster <- get_parameters(subxrace_brm)

ggplot(interaction_poster, aes(b_Drugs.11.RaceEthnicity4)) +
...
example1 <- sum(interaction_poster$b_Drugs.11.RaceEthnicity4 > 0) / nrow(interaction_poster)
example_comp <- sum(interaction_poster$b_Drugs.11.RaceEthnicity5 > 0) / nrow(interaction_poster)

q_ex1 <- quantile(interaction_poster$b_Intercept +
                  interaction_poster$b_Drugs.11 +
                  interaction_poster$b_Drugs.11.RaceEthnicity4,
         probs = c(0.025, 0.5, 0.975))

q_ex2 <- quantile(interaction_poster$b_Intercept +
                  interaction_poster$b_Drugs.11 +
                  interaction_poster$b_Drugs.11.RaceEthnicity5,
         probs = c(0.025, 0.5, 0.975))

# drugs_figure <- conditional_effects(subxrace_brm, "RaceEthnicity:Drugs.1")
# tobacco_figure <- conditional_effects(subxrace_brm, "RaceEthnicity:Tobacco.1")
three_figure <- conditional_effects(subxrace_brm_3)

# posterior_summary(subxrace_brm)

# Full set of probabilities for comparison
Drugs <- data.frame(
Asian = sum(interaction_poster$b_Drugs.11.RaceEthnicity2 > 0) / nrow(interaction_poster),
Latinx = sum(interaction_poster$b_Drugs.11.RaceEthnicity3 > 0) / nrow(interaction_poster),
Black = sum(interaction_poster$b_Drugs.11.RaceEthnicity4 > 0) / nrow(interaction_poster),
White = sum(interaction_poster$b_Drugs.11.RaceEthnicity5 > 0) / nrow(interaction_poster),
PacificIslander = sum(interaction_poster$b_Drugs.11.RaceEthnicity8 > 0) / nrow(interaction_poster),
Multi = sum(interaction_poster$b_Drugs.11.RaceEthnicity44 > 0) / nrow(interaction_poster))

Tobacco <- data.frame(
Asian = sum(interaction_poster$b_Tobacco.11.RaceEthnicity2 > 0) / nrow(interaction_poster),
Latinx = sum(interaction_poster$b_Tobacco.11.RaceEthnicity3 > 0) / nrow(interaction_poster),
Black = sum(interaction_poster$b_Tobacco.11.RaceEthnicity4 > 0) / nrow(interaction_poster),
White = sum(interaction_poster$b_Tobacco.11.RaceEthnicity5 > 0) / nrow(interaction_poster),
PacificIslander = sum(interaction_poster$b_Tobacco.11.RaceEthnicity8 > 0) / nrow(interaction_poster),
Multi = sum(interaction_poster$b_Tobacco.11.RaceEthnicity44 > 0) / nrow(interaction_poster))

Alcohol <- data.frame(
Asian = sum(interaction_poster$b_RaceEthnicity2 > 0) / nrow(interaction_poster),
Latinx = sum(interaction_poster$b_RaceEthnicity3 > 0) / nrow(interaction_poster),
Black = sum(interaction_poster$b_RaceEthnicity4 > 0) / nrow(interaction_poster),
White = sum(interaction_poster$b_RaceEthnicity5 > 0) / nrow(interaction_poster),
PacificIslander = sum(interaction_poster$b_RaceEthnicity8 > 0) / nrow(interaction_poster),
Multi = sum(interaction_poster$b_RaceEthnicity44 > 0) / nrow(interaction_poster))
...
The research question warranted a interaction model which examines the
relationships between Race/Ethnicity and substances which led to the first OSS
incident for students in grades 6-12. A Bayesian approach was selected given the
high likelihood there there would be an interaction across eight Race/Ethnicity
categories and three substance types for OSS where post-hoc comparisons can
speak to the comparison of probabilities post-hoc for different groups (e.g.,
probability for an OSS for students of one Race/Ethnicity may be compared with
students of another Race/Ethnicity directly). Earlier research questions
compared models of increasing complexity for fixed and random effects, and a
conditional model with substance type and Race/Ethnicity fixed effects that
varies randomly by school was deemed most appropriate for the current dataset.
As such, the model for this interaction research question followed a similar
construction and included the interaction among fixed effects. More
specifically, the three substance types (alcohol, tobacco, drugs) interacted
with Race/Ethnicity as fixed effects, and the outcome varied randomly by school.
It is noteworthy that at the completion of building the current model, all
$\hat{r}$ values = 1.00 and the Pacific Islander race/ethnicity group had a
sizable estimated error for one estimate (i.e., log-odds estimate of -0.61 with
credible interval ranging [-2.99 - 1.59]). Because other estimates with the
Pacific Islander race/ethnicity group had a much lower estimated error, this
group was retained for interaction analyses.

On average, the log-odds of receiving a first-time OSS for alcohol use as a
student coded Native American (i.e., race/ethnicity code 1) were 1.32 (95%
credible interval = [0.56 - 2.14]), which translates to a 21% chance (with 95%
confidence, we predict the true range to be between 11% and 37%) of receiving an
out of school suspension for this specific group. For the other substance types,
drugs and tobacco, the log-odds and therefore probabilities were demonstrably
higher; a first-time OSS for drugs was 33% (95% range: [19% - 56%]), and tobacco
was 85% (95% range: [71% - 93%]). These results per substance type are the
initial log-odds and probabilities that speak to individual parameters, but not
the interaction among parameters nor the comparisons between them.

For the interaction estimates, we will examine one example case: receiving an
OSS for drugs as a black student. The log-odds of this occurrence is 2.09 (95%
credible interval range [1.22 - 3.01]) with an associated probability of 89%. As
a basis of comparison example, let's also examine OSS for drugs as a white
student: log-odds are 1.57 (95% credible interval range [0.77 - 2.42]) with an
associated probability of 82%. For a more direct comparison of these two cases,
the difference between groups was computed based on the posterior distribution.
Black students were approximately 3 times more likely than white students to
receive a first-time OSS for drugs (20.8% likelihood for white students versus a
63.0% likelihood for black students). Specific to drugs as the substance
instigating a first time OSS, the racial/ethnic group most likely to receive
this form of discipline was Pacific Islanders (84.6%), which was approximately 4
times higher than the lowest likelihood group of receiving a first-time OSS for
drugs, White students (20.8%).
...
DA NOtes:
- drop one of the levels and keep intercept in there. Can then do post-hoc comparisons without p-values
...
# #still getting a convergence "failure", then running the model produces an object 
# 
# summary(uncond)
# summary(sub)
# summary(sub1)
