...
subxrace_brm <- brm(OSS_1stRef ~  1 + Alcohol.1 + Drugs.1 + Tobacco.1 +
                    RaceEthnicity + RaceEthnicity:Alcohol.1 +
                    RaceEthnicity:Drugs.1 + RaceEthnicity:Tobacco.1 +
                    (1|School),
             data= year7,
             ...
             cores = 2,
             backend = "cmdstan")
#took me 25 minutes to run!!

#looking at probability per substance
brms::inv_logit_scaled(fixef(subxrace_brm))
# 89% chance of [...] alcohol, 91% drugs, 46% tobacco... need help with this interpretation and how it ties in with differential probabilities (1-p???)

# now trying distributions and seeing if that helps with differentiation

# draws1 <- spread_draws(subxrace_brm, frame[RaceEthnicity,]) 
# Needs to locate RaceEthnicity somehow... and now I think I'm going too far
# base_subxrace <- tibble(substance = rep(1:3, each = length()))
...
# stuck here... "object 'Alcohol.1' not found"
pred_bysub %>% 
  mutate(
    pred = predict(subxrace3,
                   pred_bysub,
