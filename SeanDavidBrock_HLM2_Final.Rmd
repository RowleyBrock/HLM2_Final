...
# Sean to explain variables here:
Important vars-
  School : school ID
  SID : student ID
  RaceEthnicity : Categorical variable of student race
  OSS_1stRef : true/false student received OSS on first referral in school year for substance possession
  Alcohol.1, Drugs.1, Tobacco.1 : true/false student possessed named substance in 1st referral
  subtype: categorical variable of substance possessed on first referral. Mutually exclusive categories eliminate some coocurrence that happened. Probably won't use this variable, but there if needed. 

Additional vars-
  Gender: Male (0), Female(1)
  ODRCount: # of ODRs for substance in that school year for each student
  Grade : grade of student at time of referral
  IEP : T/F if student had IEP at time of referral. 
 
Then a bunch of school variables that could be interesting for other analyses with more complex models: 
  BLW_Sample: Flag for students in schools that had minimum of 5% students in each abbreviated race category            (e.g., B=Black, L=Latinx) race categories referred for substances in year. Likely not needed for these         student-level analyses
  Tier1Fidelity: school was implementing with fidelity (1) or not (0) based on one of three measures that year.
...
#Q1 
-convert log likelihood to probability for each substance in sub model to understand differential probabilities
note: intercept for sub is meaningless. probabilities compare to referrals for other substances
...
```{r q2}
Interaction equation as a start: subxrace3 <- glmer(OSS_1stRef ~  1 + Alcohol.1 * RaceEthnicity +
